import { addOneExercise } from "../../../services/api/exercise";

export const addExercise = async (data, token) => {
    /*
    Usage: thêm một bài tập cho user

    Params:
    data = {
        exercise_name: String (bắt buộc),
        exerciseTime: Time (bắt buộc),
        exercise_description: String,
        nutritionDiaryId: Number (bắt buộc)
    }
    token = String (bắt buộc)

    Return: {
        status: String,
        message: String ||     
        data = {
            "id": 9,
            "exercise_name": "chay bo",
            "exerciseTime": "07:00:01",
            "exercise_decription": "tap cho vui",
            "nutritionDiaryId": 5
        },
        code: Number
    }
    */
    try {
        const response = await addOneExercise(data, token);
        switch (response.statusCode) {
            case 400:
                // Bad request in body
                return { status: 'failed', message: 'Lỗi dữ liệu đầu vào hoặc trùng thông tin (giờ)', code: 400 };
            case 401:
                // Unauthorized
                return { status: 'failed', message: 'Lỗi xác minh, vui lòng đăng nhập lại', code: 401 };
            case 404:
                // Bad request in body
                return { status: 'failed', message: 'Không tìm thấy thông tin', code: 404 };
            case 201:
                // success
                // return data for double check
                return { status: 'success', data: response.data, code: 201 };
            case 500:
                // internal server error
                return { status: 'failed', message: 'Máy chủ đang bận, vui lòng thử lại sau!', code: 500 };
            default:
                console.log(response);
                // null/300
                return { status: 'failed', message: 'Lỗi bất định!', code: 500 };
        }
    } catch (error) {
        throw error;
    }
}