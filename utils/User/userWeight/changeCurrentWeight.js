import { updateCurrentUserWeight } from "../../../services/api/userWeight";

export const changeCurrentWeight = async (data, token) => {
    /*
    Usage: cập nhật thông tin cân nặng hiện tại của user
    ...
    */
    try {
        const response = await updateCurrentUserWeight(data, token);
        switch (response.statusCode) {
            case 400:
                // Bad request in body
                return { status: 'failed', message: 'Thông tin cung cấp không hợp lệ', code: 400 };
            case 401:
                // Unauthorized
        ...
        console.error(error, "(catch in function changeCurrentWeight)");
